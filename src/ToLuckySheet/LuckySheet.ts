...
export class LuckySheet extends LuckySheetBase {
  ...
  constructor(
    ...
    const conditionList = this.readXml.getElementsByTagName(
    ...
    const extLstRule =
      extLstCondition
        ?.map((condition) => {
          const sqref = this.readXml.getElementsByTagName(
            'xm:sqref',
            condition.value,
            false
          )?.[0];
      ...
      const ruleList =
        conditionList
          .map((condition) => {
            return this.readXml
              ...
              ?.map((d) => ({
                ...
                extLst: extLstRule.find(
                  (d: any) =>
                    d.parentAttribute.sqref === condition.attributeList?.sqref
                ),
              }));
          })
          ?.flat()
          ...
          .concat(
            extLstRule?.filter(
              (d: any) =>
                conditionList.findIndex(
                  (condition) =>
                    condition.attributeList.sqref === d.parentAttribute.sqref
                ) === -1
            )
          ) || [];

      this.conditionalFormatting = ruleList.map(
        (d: any) => new LuckyCondition(d, this.readXml, this.styles)
      );
    ...
    if (twoCellAnchors != null && twoCellAnchors.length > 0) {
      for (let i = 0; i < twoCellAnchors.length; i++) {
        ...
        if (xdrFroms != null && xdrFroms.length > 0) {
          ...
          let xdr_graphicFrame =
            twoCellAnchor.getInnerElements('xdr:graphicFrame');
          if (xdr_graphicFrame) {
  ...
  private generateConfigDataValidations(): IluckysheetDataVerification {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const matchType =
        COMMON_TYPE2.includes(_type) || !DATA_VERIFICATION_TYPE2_MAP[_type]
          ? 'common'
          : _type;
      _type2 = operator
  ...
  private getCellSize = (cell: Element) => {
    ...
    const width =
      this.config.columnlen && this.config.columnlen[col]
        ? this.config.columnlen[col]
        : this.defaultColWidth;
    const height =
      this.config.rowlen && this.config.rowlen[row]
